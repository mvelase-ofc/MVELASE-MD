      ...
      for (const jid of participants) {
         ...
         if (action == "add" && config.WELCOME ) {
           const userName = jid.split("@")[0];
                    const joinTime = moment.tz('Zimbabwe/Harare').format('HH:mm:ss');
                    const joinDate = moment.tz('Zimbabwe/Harare').format('DD/MM/YYYY');
...
"`, contextInfo: {
           ...
           const userName = jid.split('@')[0];
                    const leaveTime = moment.tz('Zimbabwe/Harare').format('HH:mm:ss');
                    const leaveDate = moment.tz('Zimbabwe/Harare').format('DD/MM/YYYY');
                    const membersCount = metadata.participants.length;
            sock.sendMessage(id, {
