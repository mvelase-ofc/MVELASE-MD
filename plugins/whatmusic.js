...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    try {
      ...
      const txt = `*ʀᴇsᴜʟᴛs*
      • 📌 *ᴛɪᴛᴛʟᴇ* : ${title}
      • 👨‍🎤 *ᴀʀᴛɪsᴛ* : ${artists ? artists.map(v => v.name).join(', ') : '*NOT FOUND*'}
      • 💾 ᴀʟʙᴜᴍ : ${album ? album.name : '*NOT FOUND*'}
      • 🌐 *ɢᴇɴʀᴇ* : ${genres ? genres.map(v => v.name).join(', ') : '*NOT FOUND*'}
      • 📆 *ʀᴇʟᴇᴀsᴇ ᴅᴀᴛᴇ* : ${release_date || '*NOT FOUND*'}
      `.trim();
